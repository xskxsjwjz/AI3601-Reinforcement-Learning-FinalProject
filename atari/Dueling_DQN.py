...
class Dueling_DQN(nn.Module):
    ...
    # 卷积 -> 展平为 [batch_size, c*h*w] -> 分别计算 V(s), A(s, a) -> 输出 Q(s, a)
    def forward(self, x):
        x = self.features(x)
...
class Dueling_DQNAgent():
    ...
    def __init__(self, input_shape, action_space=[], capacity=10000, epsilon=1.0, lr=1e-4):
        ...
        self.policy = Dueling_DQN(input_shape, action_space.n).to(self.device)
        self.target = Dueling_DQN(input_shape, action_space.n).to(self.device)
