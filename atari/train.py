...
def train(env_name):
    ...
    # 处理 frame_stack 关闭时返回 numpy.ndarray 的情况
    if hasattr(frame, 'force'):
        input_shape = frame.force().transpose(2, 0, 1).shape
    else:
        input_shape = np.array(frame).transpose(2, 0, 1).shape
    agent = Dueling_DQNAgent(input_shape, action_space, capacity=capacity, epsilon=epsilon_max, lr=lr)
    ...
    for i in range(max_frame):
        epsilon = epsilon_decay(i)  # 开始学习以后再开始衰减
